...
const Members: React.FC<MembersProps> = ({ currentSector, sectors }) => {
  ...
  const itemsForMinute = isSelectionMode && selectedIds.size > 0 
      ? displayedMembers.filter(m => selectedIds.has(m.id))
      : displayedMembers;

  return (
    <div className="space-y-6">
      
      {/* Print Header for Reports (Not Minute) */}
      <div className="print-header hidden">
        <h1 className="text-2xl font-bold uppercase">A. D. NATIVIDADE DA SERRA</h1>
        <p>Relatório de Membros - {getSectorName(currentSector)}</p>
          ...
          <div className="bg-white shadow-lg mx-auto p-12 md:p-16 max-w-[210mm] min-h-[297mm] text-justify relative animate-fade-in print:shadow-none print:w-full print:m-0">
             <div className="text-center mb-10">
                ...
                <p className="text-sm font-medium text-slate-500 mt-1 uppercase">
                   {currentSector === 'ALL' ? 'SECRETARIA GERAL' : getSectorName(currentSector)}
                </p>
                <div className="w-24 h-1 bg-slate-800 mx-auto mt-4 mb-2"></div>
                <h2 className="text-lg font-bold uppercase underline mt-6">
                   ROL OFICIAL DE MEMBROS
                </h2>
             </div>

             <div className="font-serif leading-loose text-slate-800 space-y-6">
                <p>
                    Aos <span className="font-bold">{getLongDate()}</span>, certificamos para os devidos fins eclesiásticos e administrativos que o presente documento constitui o ROL OFICIAL de membros em plena comunhão com a 
                    Igreja Evangélica Assembleia de Deus – Ministério Taubaté – Setor Natividade da Serra{currentSector !== 'ALL' ? `, congregando no setor ${getSectorName(currentSector)}` : ''}.
                </p>
                
                <p>
                    A listagem abaixo reflete o cadastro atualizado na secretaria da igreja nesta data, contendo os nomes, cargos e dados de batismo dos respectivos membros.
                </p>
                    ...
                    <table className="w-full border-collapse border border-slate-800 text-sm">
                        <thead>
                            <tr className="bg-slate-100">
                                <th className="border border-slate-600 px-2 py-1 text-center w-12">Nº</th>
                                <th className="border border-slate-600 px-2 py-1 text-left">Nome Completo</th>
                                <th className="border border-slate-600 px-2 py-1 text-center">Cargo</th>
                                <th className="border border-slate-600 px-2 py-1 text-center">Batismo</th>
                            </tr>
                        </thead>
                        <tbody>
                            {itemsForMinute.sort((a,b) => a.fullName.localeCompare(b.fullName)).map((member, index) => (
                                <tr key={member.id}>
                                    <td className="border border-slate-600 px-2 py-1 text-center">{index + 1}</td>
                                    <td className="border border-slate-600 px-2 py-1 uppercase">{member.fullName}</td>
                                    <td className="border border-slate-600 px-2 py-1 text-center">{member.role}</td>
                                    <td className="border border-slate-600 px-2 py-1 text-center">
                                        {member.isBaptized 
                                            ? (member.baptismDate ? formatDate(member.baptismDate) : 'Sim') 
                                            : 'Não'
                                        }
                                    </td>
                                </tr>
                ...
                <div className="bg-slate-50 border border-slate-200 p-4 text-sm mt-4">
                    <p className="font-bold">RESUMO ESTATÍSTICO:</p>
                    <p>Total de Membros Listados: {itemsForMinute.length}</p>
                    <p>Membros Batizados: {itemsForMinute.filter(m => m.isBaptized).length}</p>
                </div>
      ...
      <>
        ...
        {/* Members List - Report Friendly */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {itemsForMinute.map((member) => (
            <div key={member.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow break-inside-avoid page-break-inside-avoid ${isSelectionMode && selectedIds.has(member.id) ? 'border-emerald-500 ring-1 ring-emerald-500 bg-emerald-50/10' : 'border-slate-100'}`}>
